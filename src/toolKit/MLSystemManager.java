...
//		args = new String[]{"-L", "perceptron", "-A", "data/notLinearlySeparable.arff", "-E", "training", "-N"};
		...
		else if (evalMethod.equals("static"))
		{
			...
			Matrix testFeatures = new Matrix(testData, 0, 0, testData.rows(), testData.cols());
		...
		else if (evalMethod.equals("random"))
		{
			...
			Matrix trainFeatures = new Matrix(data, 0, 0, trainSize, data.cols());
			Matrix trainLabels = new Matrix(data, 0, data.cols() - 1, trainSize, 1);
			Matrix testFeatures = new Matrix(data, trainSize, 0, data.rows() - trainSize, data.cols());
		...
		else if (evalMethod.equals("cross"))
		{
			...
			for(int j = 0; j < reps; j++) {
				...
				for (int i = 0; i < folds; i++) {
					...
					Matrix trainFeatures = new Matrix(data, 0, 0, begin, data.cols());
					Matrix trainLabels = new Matrix(data, 0, data.cols() - 1, begin, 1);
					Matrix testFeatures = new Matrix(data, begin, 0, end - begin, data.cols());
